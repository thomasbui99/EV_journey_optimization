...
# charging points location on highway (km)
X = np.array([50, 250, 350, 400, 550])
...
# constraints
def f_binary(alpha): return alpha[:len(X)]*(1 - alpha[:len(X)])


binary_cons = opt.NonlinearConstraint(f_binary, 0, 0)


def f_soc_L(alpha): return a + r * np.sum(alpha[len(X):] * alpha[:len(X)])
...
def describe_journey(alpha):
    ...
    consumption = c*(L-X[-1])
    print('exit highway with {}% (aim was soc_L = {}%)'.format(
        soc_last_cp - consumption, soc_L))
    print('')
